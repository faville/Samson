...
namespace BootstrapSupport
{
    public static class DefaultScaffoldingExtensions
    {
        ...
        public static PropertyInfo[] VisibleProperties(this IEnumerable Model)
        {
            ...
            return elementType.GetProperties().Where(info => info.Name != elementType.IdentifierPropertyName() && (info.GetCustomAttribute<BrowsableAttribute>() != null ? info.GetCustomAttribute<BrowsableAttribute>().Browsable : true)).OrderedByDisplayAttr().ToArray();
        }

        public static PropertyInfo[] VisibleProperties(this Object model)
        {
            return model.GetType().GetProperties().Where(info => info.Name != model.IdentifierPropertyName() && (info.GetCustomAttribute<BrowsableAttribute>() != null ? info.GetCustomAttribute<BrowsableAttribute>().Browsable : true)).OrderedByDisplayAttr().ToArray();
        }
